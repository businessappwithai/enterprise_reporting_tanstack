...
function QueueManagementContent() {
  ...
  useCopilotAction({
    ...
    description:
      "Create a new scheduled job definition. Ask the user for the job name, type (report:generate, data:export, email:batch, scheduled:refresh), and cron schedule if not provided.",
    parameters: [
      { name: "name", type: "string", description: "Job name", required: true },
      {
        name: "jobType",
        type: "string",
        description: "Task type ID (e.g. report:generate, email:batch)",
        required: true,
      },
      {
        name: "cron",
  ...
  useCopilotAction({
    ...
    parameters: [
      {
        name: "taskId",
        type: "string",
        description: "Task ID (e.g. report:generate)",
        required: true,
      },
    ],
    handler: async ({ taskId }) => {
      ...
      if (!task)
        return `Unknown task ID: ${taskId}. Available: ${TRIGGER_TASKS.map((t) => t.id).join(", ")}`;
  ...
  useCopilotAction({
    ...
    description:
      "Explain cron schedule syntax and give examples for common patterns like daily, hourly, weekly",
    parameters: [
      {
        name: "pattern",
        type: "string",
        description: "Natural language pattern like 'daily', 'every Monday', 'hourly'",
        required: false,
      },
    ],
              ...
              <div className="space-y-2">
                {activeJobs.map((job: any) => (
                  ...
                  >
                    ...
                    <Badge
                      variant={job.schedule_cron ? "default" : "secondary"}
                      className="text-xs"
                    >
        ...
        <Card className="border-dashed">
          <CardContent className="py-4">
            <p className="text-xs text-muted-foreground text-center">
              ...
              . For local development, set <span className="font-mono">TRIGGER_API_URL</span> to
              your Mastra.ai server.
            </p>
          </CardContent>
        </Card>
      </div>
