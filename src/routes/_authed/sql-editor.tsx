...
function SQLEditorPage() {
  ...
  const handleValidate = useCallback(async () => {
    if (!sqlContent.trim()) {
      setValidationResult({ isValid: false, errors: [{ message: 'SQL query cannot be empty' }], warnings: [] })
      return
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        setValidationResult({ isValid: false, errors: [{ message: data.error?.message || 'Validation failed' }], warnings: [] })
      }
    } catch (error) {
      setValidationResult({ isValid: false, errors: [{ message: error instanceof Error ? error.message : 'Unknown error' }], warnings: [] })
    } finally {
                ...
                <div className="mt-2">
                  ...
                  <ul className="list-disc list-inside ml-2">
                    {validationResult.warnings.map((warning, idx) => (
                      <li key={idx} className="text-xs">{warning.message} ({warning.type})</li>
                    ))}
                  </ul>
                </div>
              )}
            ...
            <div className="text-sm text-red-700">
              ...
              <ul className="list-disc list-inside ml-2">
                {validationResult.errors.map((error, idx) => (
                  <li key={idx}>{error.message}{error.line !== undefined && ` (line ${error.line})`}</li>
                ))}
              </ul>
              {validationResult.warnings.length > 0 && (
                <div className="mt-2">
            ...
            <div>
              ...
              {warning && (
                <div className="border border-yellow-300 bg-yellow-50 dark:bg-yellow-900/20 rounded p-4">
                  ...
                  <p className="text-sm text-yellow-600 dark:text-yellow-300">Suggestion: {warning.suggestion}</p>
                </div>
              )}
