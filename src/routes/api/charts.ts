    ...
    handlers: {
      GET: async ({ request }) => {
        try {
          ...
          return json({
            success: true,
            data: { items: paginatedCharts, meta: { total, page, pageSize, totalPages: Math.ceil(total / pageSize) } },
          })
        } catch (error) {
          console.error('Error fetching charts:', error)
          return json({ success: false, error: { code: 'SERVER_ERROR', message: 'Failed to fetch charts' } }, { status: 500 })
        }
      },
      ...
      POST: async ({ request }) => {
        try {
          ...
          if (!canCreate) {
            return json({ success: false, error: { code: 'FORBIDDEN', message: 'You do not have permission to create charts' } }, { status: 403 })
          }
          ...
          if (!name || !chartType) {
            return json({ success: false, error: { code: 'INVALID_INPUT', message: 'Name and chart type are required' } }, { status: 400 })
          }
          ...
          await logAudit({ userId: session.user.id, action: 'create', resourceType: 'chart', resourceId: id, details: { name, chartType } })

          const chart = await db<ChartDefinition>('chart_definitions').where('id', id).first()
          return json({ success: true, data: chart })
        } catch (error) {
          console.error('Error creating chart:', error)
          return json({ success: false, error: { code: 'SERVER_ERROR', message: 'Failed to create chart' } }, { status: 500 })
        }
      },
    },
  },
