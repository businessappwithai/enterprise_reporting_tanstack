    ...
    handlers: {
      GET: async ({ request, params }) => {
        try {
          ...
          const filters = await db('chart_filters as cf')
            ...
            .select('cf.id', 'cf.chart_id', 'cf.filter_id', 'cf.target_column', 'cf.filter_order', 'fd.name as filter_name', 'fd.description', 'fd.data_source_id', 'fd.filter_query', 'fd.display_field', 'fd.value_field')
      ...
      POST: async ({ request, params }) => {
        try {
          ...
          const maxOrderResult = await db('chart_filters').where('chart_id', params.id).max('filter_order as max_order').first()
          const nextOrder = (maxOrderResult?.max_order ?? -1) + 1

          const newChartFilter = { id: randomUUID(), chart_id: params.id, filter_id, target_column, filter_order: nextOrder, created_at: new Date().toISOString() }
