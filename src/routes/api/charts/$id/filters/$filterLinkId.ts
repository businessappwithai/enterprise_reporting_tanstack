    ...
    handlers: {
      PUT: async ({ request, params }) => {
        try {
          ...
          const filterLink = await db('chart_filters').where('id', params.filterLinkId).where('chart_id', params.id).update(updateData)
      ...
      DELETE: async ({ request, params }) => {
        try {
          ...
          const { getConfigDB } = await import('@/lib/db/config')
          const db = getConfigDB()
          const filterLink = await db('chart_filters').where('id', params.filterLinkId).where('chart_id', params.id).del()
          if (!filterLink) return json({ error: 'Chart filter not found' }, { status: 404 })
