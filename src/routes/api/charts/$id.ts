    ...
    handlers: {
      GET: async ({ request, params }) => {
        try {
          ...
          if (!hasAccess) {
            return json({ success: false, error: { code: 'FORBIDDEN', message: 'You do not have permission to view this chart' } }, { status: 403 })
          }
          ...
          if (!chart) {
            return json({ success: false, error: { code: 'NOT_FOUND', message: 'Chart not found' } }, { status: 404 })
          }

          return json({ success: true, data: chart })
        } catch (error) {
          console.error('Error fetching chart:', error)
          return json({ success: false, error: { code: 'SERVER_ERROR', message: 'Failed to fetch chart' } }, { status: 500 })
        }
      },
      ...
      PUT: async ({ request, params }) => {
        try {
          ...
          if (!hasAccess) {
            return json({ success: false, error: { code: 'FORBIDDEN', message: 'You do not have permission to edit this chart' } }, { status: 403 })
          }
          ...
          await logAudit({ userId: session.user.id, action: 'update', resourceType: 'chart', resourceId: id })

          const chart = await db<ChartDefinition>('chart_definitions').where('id', id).first()
          return json({ success: true, data: chart })
        } catch (error) {
          console.error('Error updating chart:', error)
          return json({ success: false, error: { code: 'SERVER_ERROR', message: 'Failed to update chart' } }, { status: 500 })
        }
      },
      ...
      DELETE: async ({ request, params }) => {
        try {
          ...
          if (!canDelete) {
            return json({ success: false, error: { code: 'FORBIDDEN', message: 'You do not have permission to delete this chart' } }, { status: 403 })
          }
          ...
          await logAudit({ userId: session.user.id, action: 'delete', resourceType: 'chart', resourceId: id })

          return json({ success: true })
        } catch (error) {
          console.error('Error deleting chart:', error)
          return json({ success: false, error: { code: 'SERVER_ERROR', message: 'Failed to delete chart' } }, { status: 500 })
        }
      },
    },
  },
