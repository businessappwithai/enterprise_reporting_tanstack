    ...
    handlers: {
      GET: async ({ request, params }) => {
        try {
          ...
          const db = getDb()
          const widgets = await db<DashboardWidget>('dashboard_widgets').where('dashboard_id', dashboardId).orderBy('created_at')
          return json({ success: true, data: { items: widgets } })
        } catch (error) {
          console.error('Error fetching widgets:', error)
          return json({ success: false, error: { code: 'SERVER_ERROR', message: 'Failed to fetch widgets' } }, { status: 500 })
        }
      },
      ...
      POST: async ({ request, params }) => {
        try {
          ...
          if (!widgetType || !positionConfig) {
            return json({ success: false, error: { code: 'INVALID_INPUT', message: 'widgetType and positionConfig are required' } }, { status: 400 })
          }
          ...
          await logAudit({ userId: session.user.id, action: 'create', resourceType: 'dashboard_widget', resourceId: id, details: { dashboardId, widgetType } })

          const widget = await db<DashboardWidget>('dashboard_widgets').where('id', id).first()
          return json({ success: true, data: widget })
        } catch (error) {
          console.error('Error creating widget:', error)
          return json({ success: false, error: { code: 'SERVER_ERROR', message: 'Failed to create widget' } }, { status: 500 })
        }
      },
    },
  },
