    ...
    handlers: {
      GET: async ({ request, params }) => {
        try {
          ...
          if (!hasAccess) {
            return json({ success: false, error: { code: 'FORBIDDEN', message: 'You do not have permission to view this dashboard' } }, { status: 403 })
          }
          ...
          if (!dashboard) {
            return json({ success: false, error: { code: 'NOT_FOUND', message: 'Dashboard not found' } }, { status: 404 })
          }

          const widgets = await db<DashboardWidget>('dashboard_widgets').where('dashboard_id', id).orderBy('created_at')
          return json({ success: true, data: { ...dashboard, widgets } })
        } catch (error) {
          console.error('Error fetching dashboard:', error)
          return json({ success: false, error: { code: 'SERVER_ERROR', message: 'Failed to fetch dashboard' } }, { status: 500 })
        }
      },
      ...
      PUT: async ({ request, params }) => {
        try {
          ...
          if (!hasAccess) {
            return json({ success: false, error: { code: 'FORBIDDEN', message: 'You do not have permission to edit this dashboard' } }, { status: 403 })
          }
          ...
          await logAudit({ userId: session.user.id, action: 'update', resourceType: 'dashboard', resourceId: id })
          ...
          return json({ success: false, error: { code: 'SERVER_ERROR', message: 'Failed to update dashboard' } }, { status: 500 })
        }
      },
      ...
      DELETE: async ({ request, params }) => {
        try {
          ...
          if (!canDelete) {
            return json({ success: false, error: { code: 'FORBIDDEN', message: 'You do not have permission to delete this dashboard' } }, { status: 403 })
          }
          ...
          await logAudit({ userId: session.user.id, action: 'delete', resourceType: 'dashboard', resourceId: id })

          return json({ success: true })
        } catch (error) {
          console.error('Error deleting dashboard:', error)
          return json({ success: false, error: { code: 'SERVER_ERROR', message: 'Failed to delete dashboard' } }, { status: 500 })
        }
      },
    },
  },
