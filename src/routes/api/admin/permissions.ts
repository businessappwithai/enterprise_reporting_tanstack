    ...
    handlers: {
      GET: async ({ request }) => {
        try {
          ...
          return json(
            { success: false, error: { message: "Failed to fetch permissions" } },
            { status: 500 }
          );
        }
      },
      ...
      POST: async ({ request }) => {
        try {
            ...
            return json(
              {
                success: false,
                error: {
                  message: "resourceType, resourceId, roleId, and permissionLevel are required",
                },
              },
              { status: 400 }
            );
          ...
          return json(
            { success: false, error: { message: "Failed to create permission" } },
            { status: 500 }
          );
        }
      },
      ...
      DELETE: async ({ request }) => {
        try {
          ...
          return json(
            { success: false, error: { message: "Failed to delete permission" } },
            { status: 500 }
          );
        }
      },
