    ...
    handlers: {
      GET: async ({ request }) => {
        try {
          ...
          return json({ success: false, error: { code: 'SERVER_ERROR', message: 'Failed to fetch queries' } }, { status: 500 })
        }
      },
      ...
      POST: async ({ request }) => {
        try {
          ...
          if (!name || !dataSourceId || !sqlContent) {
            return json({ success: false, error: { code: 'INVALID_INPUT', message: 'Name, dataSourceId, and sqlContent are required' } }, { status: 400 })
          }
          ...
          return json({ success: false, error: { code: 'SERVER_ERROR', message: 'Failed to create query' } }, { status: 500 })
        }
      },
    },
  },
