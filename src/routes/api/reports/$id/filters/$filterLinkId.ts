    ...
    handlers: {
      PUT: async ({ request, params }) => {
        try {
          ...
          const filterLink = await db('report_filters').where('id', filterLinkId).where('report_id', reportId).update(updateData)
      ...
      DELETE: async ({ request, params }) => {
        try {
          ...
          const { getConfigDB } = await import('@/lib/db/config')
          const db = getConfigDB()
          const filterLink = await db('report_filters').where('id', filterLinkId).where('report_id', reportId).del()
