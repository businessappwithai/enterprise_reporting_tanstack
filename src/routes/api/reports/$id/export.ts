    ...
    handlers: {
      POST: async ({ request, params }) => {
        try {
          ...
          if (!isFormatEnabled) {
            return json({ success: false, error: { code: 'FORBIDDEN', message: `${format.toUpperCase()} export is not enabled for this report` } }, { status: 403 })
          }

          if (!report.saved_query_id) {
            return json({ success: false, error: { code: 'NO_QUERY', message: 'Report has no associated query' } }, { status: 400 })
          }

          const query = await db<SavedQuery>('saved_queries').where('id', report.saved_query_id).first()
          if (!query) {
            return json({ success: false, error: { code: 'QUERY_NOT_FOUND', message: 'Associated query not found' } }, { status: 404 })
          }

          const dataSource = await db<DataSource>('data_sources').where('id', query.data_source_id).where('is_active', true).first()
          if (!dataSource) {
            return json({ success: false, error: { code: 'DATASOURCE_NOT_FOUND', message: 'Data source not found' } }, { status: 404 })
          }

          if (!isReadOnlyQuery(query.sql_content)) {
            return json({ success: false, error: { code: 'INVALID_QUERY', message: 'Report query must be a SELECT query' } }, { status: 400 })
          }
          ...
          if (report.filter_config) {
            try { filterConfig = JSON.parse(report.filter_config) } catch { /* ignore */ }
          }
          ...
          if (report.column_config) {
            try { columnConfig = JSON.parse(report.column_config) } catch { /* ignore */ }
          }
          ...
          if (columnConfig.length > 0) {
            ...
            filteredRows = rows.map(row => {
              ...
              visibleColumns.forEach(col => { if (col.field in row) filteredRow[col.field] = row[col.field] })
              return filteredRow
            })
          } else {
          ...
          if (format === 'excel' || format === 'xlsx') {
            ...
            headerRow.eachCell(cell => {
              cell.font = { bold: colorTheme?.headerFontWeight === 'bold' || colorTheme?.headerFontWeight === '700', color: { argb: hexToARGB(colorTheme?.headerTextColor) } }
              cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: hexToARGB(colorTheme?.headerBackgroundColor) } }
              cell.border = { top: { style: 'thin', color: { argb: hexToARGB(colorTheme?.borderColor) } }, left: { style: 'thin', color: { argb: hexToARGB(colorTheme?.borderColor) } }, bottom: { style: 'thin', color: { argb: hexToARGB(colorTheme?.borderColor) } }, right: { style: 'thin', color: { argb: hexToARGB(colorTheme?.borderColor) } } }
            })
            ...
            filteredRows.forEach((row, rowIndex) => {
              ...
              dataRow.eachCell(cell => {
                cell.font = { color: { argb: hexToARGB(isAltRow ? colorTheme?.alternatingRowTextColor : colorTheme?.rowTextColor) } }
                cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: hexToARGB(isAltRow ? colorTheme?.alternatingRowBackgroundColor : colorTheme?.rowBackgroundColor) } }
                cell.border = { top: { style: 'thin', color: { argb: hexToARGB(colorTheme?.borderColor) } }, left: { style: 'thin', color: { argb: hexToARGB(colorTheme?.borderColor) } }, bottom: { style: 'thin', color: { argb: hexToARGB(colorTheme?.borderColor) } }, right: { style: 'thin', color: { argb: hexToARGB(colorTheme?.borderColor) } } }
              })
            })
          ...
          if (format === 'pdf') {
            ...
            const drawCell = (x: number, y: number, width: number, height: number, text: string, bgColor: { r: number; g: number; b: number }, textColor: { r: number; g: number; b: number }, isBold = false) => {
            ...
            headers.forEach((header, index) => {
              drawCell(margin + index * columnWidth, yPosition, columnWidth, rowHeight, String(header), headerBgColor, headerTextColor, headerFontBold)
            })
            yPosition += rowHeight
            ...
            filteredRows.forEach((row, rowIndex) => {
              ...
              headers.forEach((header, colIndex) => {
                ...
                const value = row[field]
                const stringValue = value === null || value === undefined ? '' : String(value)
                drawCell(margin + colIndex * columnWidth, yPosition, columnWidth, rowHeight, stringValue, rowBgColor, rowTextColor, false)
              })
              yPosition += rowHeight
            })
            ...
            for (let i = 1; i <= totalPages; i++) {
              ...
              doc.text(`Page ${i} of ${totalPages} | Total rows: ${filteredRows.length}`, pageWidth / 2, pageHeight - 5, { align: 'center' })
            }
          ...
          return json({ success: false, error: { code: 'INVALID_FORMAT', message: 'Invalid export format' } }, { status: 400 })
        } catch (error) {
          console.error('Error exporting report:', error)
          return json({ success: false, error: { code: 'SERVER_ERROR', message: 'Failed to export report' } }, { status: 500 })
        }
      },
    },
  },
