...
export function VoiceInput() {
  ...
  useEffect(() => {
    ...
    const findTargets = () => {
      if (!mounted) return;
      const inputControls = document.querySelector(".copilotKitInputControls") as HTMLElement | null;
      const messagesFooter = document.querySelector(".copilotKitMessagesFooter") as HTMLElement | null;
  ...
  const startRecording = useCallback(async () => {
    ...
    try {
      ...
      setErrorMsg("Microphone access denied. Please allow mic access in your browser settings and try again, or type your question in the chat.");
      setVoiceState("idle");
      return;
    }
  ...
  const micButton = (
    <button
      ...
      title={voiceState === "idle" ? "Voice input" : voiceState === "recording" ? "Stop recording" : "Transcribing..."}
  ...
  const stepsPanel = steps.length > 0 ? (
    <div className="voice-steps-panel" style={{
      padding: "8px 12px",
      margin: "4px 8px",
      ...
      <div style={{ marginLeft: "20px", opacity: 0.8 }}>
        {steps.map((s) => (
          <div key={s.name} style={{ display: "flex", alignItems: "center", gap: "4px", lineHeight: "1.6" }}>
            {stepIcon(s.status)}
            <span>
      ...
      {transcribedText && (
        <div style={{
          ...
          <div style={{ fontSize: "10px", fontWeight: 500, color: "hsl(142 76% 30%)", marginBottom: "2px" }}>
            Transcribed:
          </div>
