  ...
  return (
    <div className="space-y-3">
      ...
      >
        {uploading ? (
          <>
            <Loader2 className={`mx-auto mb-2 animate-spin text-muted-foreground ${compact ? 'h-6 w-6' : 'h-8 w-8'}`} />
            <p className="text-sm font-medium">Uploading{compact ? '...' : ' file...'}</p>
          </>
        ) : (
          <>
            <FolderOpen className={`mx-auto mb-2 text-muted-foreground ${compact ? 'h-6 w-6 mb-1' : 'h-8 w-8'}`} />
            <p className="text-sm font-medium">
              {fileName || (compact ? 'Click to browse or drag & drop to change file' : 'Click to browse or drag & drop SQLite file')}
            </p>
            {!compact && (
