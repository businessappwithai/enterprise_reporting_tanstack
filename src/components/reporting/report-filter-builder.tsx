...
export function ReportFilterBuilder({ filters, availableFields, onChange }: ReportFilterBuilderProps) {
  const addCondition = (groupId: string) => {
    ...
    const updateGroup = (group: FilterGroup): FilterGroup => {
      if (group.id === groupId) return { ...group, conditions: [...group.conditions, newCondition] };
  ...
  const updateCondition = (groupId: string, conditionId: string, updates: Partial<FilterCondition>) => {
    const updateGroup = (group: FilterGroup): FilterGroup => {
      if (group.id === groupId) {
        return { ...group, conditions: group.conditions.map((c) => c.id === conditionId ? { ...c, ...updates } : c) };
      }
      if (group.groups) return { ...group, groups: group.groups.map(updateGroup) };
      return group;
    };
  ...
  const removeCondition = (groupId: string, conditionId: string) => {
    const updateGroup = (group: FilterGroup): FilterGroup => {
      if (group.id === groupId) return { ...group, conditions: group.conditions.filter((c) => c.id !== conditionId) };
      if (group.groups) return { ...group, groups: group.groups.map(updateGroup) };
      return group;
    };
    onChange(updateGroup(filters));
