...
async function introspectPostgres(
  ...
  const viewInfos: ViewInfo[] = await Promise.all(
    views.rows.map(
      async (v: { table_name: string; table_schema: string; view_definition: string }) => {
...
async function getPostgresForeignKeys(
  ...
  return result.rows.map(
    (r: { column_name: string; referenced_table: string; referenced_column: string }) => ({
...
async function introspectMySQL(
  ...
  const viewInfos: ViewInfo[] = views[0].map(
    (v: { table_name: string; view_definition: string }) => ({
      name: v.table_name,
      columns: [],
      definition: v.view_definition,
    })
...
async function getMySQLColumns(connection: Knex, table: string): Promise<ColumnSchema[]> {
  ...
  return result[0].map(
    (c: { Field: string; Type: string; Null: string; Default: string | null; Key: string }) => ({
      name: c.Field,
      type: c.Type,
      nullable: c.Null === "YES",
...
async function introspectMSSQL(
  ...
  const viewInfos: ViewInfo[] = views.map(
    (v: { table_name: string; table_schema: string; view_definition: string }) => ({
      name: v.table_name,
      schema: v.table_schema,
      columns: [],
      definition: v.view_definition,
