...
import { reverseTranslateSql, assessTranslationConfidence, type SchemaMetadata } from '@/lib/validation/translation-validator'
...
export const executeNLQuery = createServerFn({
  ...
  try {
    ...
    if (!antlrValidation.valid) {
      const errorMsg = antlrValidation.errors
        .map(e => e.message)
        .join('; ')
      return {
        success: false,
    ...
    const reverseTranslation = await reverseTranslateSql(
      generatedSQL,
      nlQuestion,
      schema
    )

    const confidenceAssessment = assessTranslationConfidence(reverseTranslation, CONFIDENCE_THRESHOLD)
...
export const executeNLQueryWithOverride = createServerFn({
  method: 'POST',
}).handler(async (input: ExecuteNLQueryInput & { approvedSQL: string }): Promise<ExecuteNLQueryResult> => {
  const session = await requireAuth()
