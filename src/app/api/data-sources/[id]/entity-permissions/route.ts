...
const VALID_PERMISSION_LEVELS: DsEntityPermissionLevel[] = [
  "select",
  "insert",
  "update",
  "delete",
  ...
  try {
    ...
    if (!ds_role_id || !entity_name) {
      return NextResponse.json(
        {
          success: false,
          error: { code: "VALIDATION_ERROR", message: "ds_role_id and entity_name are required" },
        },
        { status: 400 }
      );
    }
      ...
      return NextResponse.json(
        {
          ...
          error: {
            ...
            message: `Invalid permission_level. Must be one of: ${VALID_PERMISSION_LEVELS.join(", ")}`,
          },
        },
        { status: 400 }
      );
  ...
  try {
    ...
    if (!permissionId) {
      return NextResponse.json(
        {
          success: false,
          error: { code: "VALIDATION_ERROR", message: "permission_id query param is required" },
        },
        { status: 400 }
      );
    }
