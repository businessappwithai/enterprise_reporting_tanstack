  ...
  try {
    ...
    if (queryParams.limit < 1 || queryParams.limit > 500) {
      return NextResponse.json(
        {
          success: false,
          error: { code: "VALIDATION_ERROR", message: "limit must be between 1 and 500" },
        },
        { status: 400 }
      );
    }
    ...
    const result = await DataService.listRecords(params.id, entity, queryParams, context.userId);
