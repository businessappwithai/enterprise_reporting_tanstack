...
export async function GET(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    const dataSource = await db("data_sources").where("id", id).where("is_deleted", false).first();
    ...
    const usage = {
      ...
      total:
        Number(queryCount?.count || 0) +
        Number(reportCount?.count || 0) +
        Number(chartCount?.count || 0),
    };
