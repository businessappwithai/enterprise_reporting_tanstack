...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const canView =
      hasPermission(context, "data_source", "view") ??
    ...
    const dataSource = (await getDb()("data_sources").where("id", params.id).first()) as
      | DataSource
      | undefined;
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const canAdmin =
      hasPermission(context, "data_source", "admin") ??
    ...
    if (is_editable === undefined || typeof is_editable !== "boolean") {
      return NextResponse.json(
        {
          success: false,
          error: { code: "VALIDATION_ERROR", message: "is_editable must be a boolean" },
        },
        { status: 400 }
      );
    }
    ...
    // Update datasource
    await getDb()("data_sources").where("id", params.id).update({ is_editable });
