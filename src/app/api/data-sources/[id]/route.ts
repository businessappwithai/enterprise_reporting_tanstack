...
export async function GET(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    log.apiRequest(method, path, undefined, {
    ...
    log.info("Decrypting connection config for data source", {
      dataSourceId: id,
    ...
    try {
      ...
      if (isEncrypted) {
        ...
        log.warn(
          "Data source has plain JSON config (not encrypted) - using backwards compatibility",
          {
            dataSourceId: id,
            name: dataSource.name,
          }
