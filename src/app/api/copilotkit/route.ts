...
export const POST = async (req: NextRequest) => {
  ...
  console.log(
    `[COPILOTKIT ${requestId}] OPENAI_API_KEY length: ${process.env.OPENAI_API_KEY?.length || 0}`
  );
  console.log(
    `[COPILOTKIT ${requestId}] OPENAI_MODEL: ${process.env.OPENAI_MODEL || "gpt-4o (default)"}`
  );
  ...
  try {
    ...
    try {
      ...
      console.log(
        `[COPILOTKIT ${requestId}] Response body preview:`,
    ...
    console.error(
      `[COPILOTKIT ${requestId}] Error name:`,
      error instanceof Error ? error.name : typeof error
    );
    console.error(
      `[COPILOTKIT ${requestId}] Error message:`,
      error instanceof Error ? error.message : String(error)
    );
    console.error(
      `[COPILOTKIT ${requestId}] Error stack:`,
