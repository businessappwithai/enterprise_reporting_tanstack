...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json({ error: "Failed to fetch chart filters" }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json({ error: "Failed to add chart filter" }, { status: 500 });
  }
}
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json({ error: "Failed to delete chart filters" }, { status: 500 });
  }
}
