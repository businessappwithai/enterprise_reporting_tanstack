...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const filter = await db("filter_definitions").where("id", params.id).first();
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { name, description, data_source_id, filter_query, display_field, value_field } = body;
    ...
    const db = getConfigDB();
    const filter = await db("filter_definitions").where("id", params.id).update(updateData);
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const db = getConfigDB();
    const filter = await db("filter_definitions").where("id", params.id).del();
