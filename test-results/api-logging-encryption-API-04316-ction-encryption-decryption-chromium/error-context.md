...
- Name: api-logging-encryption.spec.ts >> API - Encryption & Decryption >> should handle MySQL connection encryption/decryption
- Location: e2e/api-logging-encryption.spec.ts:132:3
...
> 149 |     expect(createResponse.status()).toBe(201);
  ...
  221 |           SELECT 2, 'Bob'
  222 |         ),
  223 |         orders(id, user_id, amount) AS (
  224 |           SELECT 1, 1, 100
  225 |           UNION ALL
  226 |           SELECT 2, 1, 200
  227 |           UNION ALL
  228 |           SELECT 3, 2, 150
  229 |         )
  230 |         SELECT u.name, COUNT(o.id) as order_count, SUM(o.amount) as total
  231 |         FROM users u
  232 |         LEFT JOIN orders o ON u.id = o.user_id
  233 |         GROUP BY u.id, u.name
  234 |         ORDER BY total DESC
  235 |       `,
  236 |       dataSourceId: sqliteDataSourceId,
  237 |     });
  238 | 
  239 |     expect(response.status()).toBe(200);
  240 | 
  241 |     const data = await ApiTestHelpers.extractJson(response);
  242 |     expect(data.success).toBe(true);
  243 |     expect(data.data.rows.length).toBeGreaterThan(0);
  244 |   });
  245 | 
  246 |   test('should execute query with aggregations', async ({ request }) => {
  247 |     const apiHelpers = new ApiTestHelpers(request, authCookie);
  248 |     const response = await apiHelpers.executeSql({
  249 |       sql: `
```