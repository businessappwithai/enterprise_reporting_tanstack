...
async function testAdminPermissions() {
  ...
  try {
    ...
    const passwordValid = await checkPassword(db, admin.id, "admin");
    ...
    console.log(
      "✓ Roles:",
      perms.roles.map((r) => r.name)
    );
    console.log("✓ Role permissions:", perms.rolePermissions);
    console.log("✓ Number of permissions:", perms.rolePermissions.length);

    const hasUserView = perms.rolePermissions.some(
      (p) => p === "user:view" || p === "user:*" || p === "admin:*" || p === "*:*"
    );
    const hasRoleView = perms.rolePermissions.some(
      (p) => p === "role:view" || p === "role:*" || p === "admin:*" || p === "*:*"
    );
