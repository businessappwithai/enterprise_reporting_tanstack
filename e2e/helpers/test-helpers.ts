...
export class TestHelpers {
  ...
  async selectDataSource(dataSourceName?: string) {
    ...
    await this.page.evaluate((element) => element.onclick(), await selectTrigger.elementHandle());
    ...
    if (dataSourceName) {
      ...
      await option.waitFor({ state: 'visible', timeout: 5000 });
      await this.page.evaluate((el) => el.onclick(), await option.elementHandle());
    } else {
      const firstOption = this.page.locator('[role="option"]').first();
      await firstOption.waitFor({ state: 'visible', timeout: 5000 });
      await this.page.evaluate((el) => el.onclick(), await firstOption.elementHandle());
    }
